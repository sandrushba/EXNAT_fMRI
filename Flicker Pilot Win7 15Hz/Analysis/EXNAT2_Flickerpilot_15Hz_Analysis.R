...
path_data_folder <- "Data/All Data/" # use 12 Hz and 15 Hz flicker datasets
#path_data_folder <- "Data/15 Hz Data/" # use 12 Hz and 15 Hz flicker datasets
...
for (i in 1:length(file_list)) {
  ...
  # if it is one of the 12 Hz pilot datasets where I asked Hiwis to play participants, use 12 Hz flicker, else 15 Hz
  if (id %in% c("at", "eg", "hs", "hr", "nd", "hf")) {
    flicker_freq = "12 Hz (but unstable)"
  } else {
    flicker_freq = "15 Hz"
  }
  ...
  df_demogr <- as.data.frame(rbind(df_demogr,
                                   cbind(id, age, gender, handedness, native_speaker, flicker_freq)))
  ...
  # add new flicker frequency column 
  subj_df$flicker_freq <- flicker_freq
