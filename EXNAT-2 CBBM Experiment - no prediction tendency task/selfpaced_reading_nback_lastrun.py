...
expInfo = {'participant': 'part0000', 'age': '', 'handedness': 'r', 'gender': 'w', 'testing_mode': 'no', 'meas_hearing': 'no', 'test_triggers': 'no', 'start_at_vistask': 'no', 'skip_prediction_tendency': 'yes'}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\AC_STIM\\Documents\\Experimentskripte\\EXNAT-2\\selfpaced_reading_nback_lastrun.py',
...
prefs.hardware['audioLib'] = ['sounddevice'] # set 'ptb', 'pyo' or'pygame' as sound library here
...
import soundfile as sf # for saving sine waves as wav files
...
# one for 1-back and 1 for 2-back à 300 trials each (50 targets):
# 2 * single main
...
blocks_textlen = [159, 300, 6, # reading bl blocks + click training
                  20, 20, 300, 300, # main block 1 trainings, single task (300 trials) & dual-task (300 trials)
                  20, 20, 300, 300, # main block 2 trainings, single tasks (300 trials) & dual-task (300 trials)
                  300, 300, 300, 300, 300, 300] # main blocks 2        
blocks_target_counts = [25, 50, 1, # reading bl blocks + click training
                        5, 5, 50, 50, # main block 1 trainings, single task & main block
                        5, 5,50, 50, # main block 2 trainings, single task & main block
                        50, 50, 50, 50, 50, 50]
...
# Initialize components for Routine "warning_2"
warning_2Clock = core.Clock()
...
#port = parallel.setPortAddress(0xB010)# set address of port --> this was for the old Win7 PC
port = parallel.setPortAddress(0xCFF8)# set address of port
...
def send_trigger(event_name):
    ...
    core.wait(time_after_trigger) # you need a break between the triggers: wait for 3 ms
...
for i in [num for num in range(2, 69) if num % 2 == 0]:
    # send trigger:
    parallel.setData(i)
    core.wait(time_after_trigger) # wait for a few ms
    # turn off trigger
    parallel.setData(0)
    ...
    core.wait(0.1) # wait 100 ms between triggers
...
for thisBlock in blocks:
    ...
    # First, check if we want to skip all main blocks to go directly to the visual task
    # (this is just for testing purposes).
    if expInfo["start_at_vistask"] == "yes":
        blocks.finished = True # end the whole "blocks" loop
    ...
    # ------Prepare to start Routine "warning_2"-------
    continueRoutine = True
    # update component parameters for each repeat
    ...
    # keep track of which components have finished
    warning_2Components = []
    for thisComponent in warning_2Components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
    warning_2Clock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    
    # -------Run Routine "warning_2"-------
    while continueRoutine:
        # get current time
        t = warning_2Clock.getTime()
        tThisFlip = win.getFutureFlipTime(clock=warning_2Clock)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
    ...
    # -------Ending Routine "warning_2"-------
    for thisComponent in warning_2Components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # the Routine "warning_2" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
...
if expInfo["start_at_vistask"] == "yes":
    vis_task_durations = [332, 10, 104.778, 98.9, 3000, 440] # in ms - I included a longer break and a very short RT as a test case for the data cleaning part later
    vis_task_words = ["Hallo,", "das", "ist", "nur", "ein", "Beispiel."]
...
# send block onset trigger
send_trigger("visual_task_training_onset")
core.wait(0.1) #wait 100ms before starting first trial
...
# Hint: We take the average reading time / letter from the vis task 
# training block to calculate reading times for each word in this block.
...
instr_text1 = "Instruktionen\n\n\nGut gemacht!\n\nNun folgt ein etwas längerer Hauptblock, die Aufgabe bleibt aber die Gleiche.\n\nBitte drücken Sie die Taste 'C', wenn Sie die folgende Farbe sehen:"
instr_text2 = "\n\nDrücken Sie die Leertaste, wenn Sie bereit sind, den Hauptblock zu starten!"
...
# send block onset trigger
send_trigger("visual_task_main_onset")
core.wait(0.1) # wait 100 ms
...
for trial_idx, curr_word in enumerate(curr_text):
    ...
    # get duration for current word
    curr_duration = vis_task_durations[trial_idx] / 1000 # convert ms to seconds
...
# go to next block!
print("finished vis task main - going to pred tendency task or end")
exp_block_counter += 1
continueRoutine = False
# keep track of which components have finished
vistask_mComponents = []
...
while True:
    ...
    if 'space' in event.getKeys():
        ...
        ### STOP EYETRACKER:
        et_abort_exp() # shut down eyetrigger and download data
