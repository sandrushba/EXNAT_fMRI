...
for (i in 1:length(file_list)) {
  ...
  # get name of csv for current participant:
  curr_file <- list.files(path = paste(path_data_folder, file_list[i], "/", sep = ""), pattern = ".csv")

  # read in current file
  subj_df <- read.csv(paste(path_data_folder, file_list[i], "/", curr_file, sep = ""), sep = ",")
  ...
  native_speaker <- TRUE # we only test native speakers
  ...
  df_demogr <- as.data.frame(rbind(df_demogr,
                                   cbind(id, age, gender, handedness, native_speaker, flicker_freq, excl)))
