...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='E:\\EXNAT-2\\EEG_study_EXNAT2\\Flicker Timing Test Pilot Win7\\selfpaced_reading_nback_lastrun.py',
...
#flicker_freq = 17.3 # doesn't work with 60 Hz display
#flicker_freq = 59/3 # 19.667 Hz --> 59 Hz display is actually a 60Hz display, so doesn't work either
flicker_freq = frame_rate/4 # 15 Hz --> is displayed as 14.981 Hz
#flicker_freq = frame_rate/3 # 20 Hz --> is displayed as 20 Hz, but there are some gaps in the text stims 
...
dark_bg_col  = [(x / 127.5) - 1 for x in (255, 255, 255)] # dark grey background for stimuli (use RGB -1:1)

# make background light for a start - use rgb -1:1 colour codes
win.setColor(light_bg_col, colorSpace='rgb')

# set colours you want to use for the stimuli:
#--> use all black here for flicker timing test
colours = ["#000000", "#0e0e0e", "#151515", "#070707"]
...
for thisBlock in blocks:
    ...
    # if it's one of the "normal" main blocks, prepare main block stimuli:
        ...
        instr_text_stim = visual.TextStim(win, 
                                          text = instr_text, 
                                          height = 0.5, # font height 0.5 = 5° visual angle
                                          font = "Bookman Old Style",
                                          pos = (0, 2),
                                          color = "black")
    ...
    stim = visual.TextStim(win = win, 
                           ...
                           height = 5) # font height = 3° visual angle
